						...
						if ( isset($GLOBALS['wp_filter'][$hook]) ) {
							...
							foreach ( (array)$GLOBALS['wp_filter'][$hook] as $priority => $function ) {
								foreach ( $function as $hook_details )
									...
									if( is_array( $hook_details['function'] ) ) {
										if( is_object( $hook_details['function'][0] ) ) {
											$hook_details['class'] = get_class( $hook_details['function'][0] );
											$hook_details['function'] = $hook_details['function'][1];
										}elseif( is_string( $hook_details['function'][0] ) ) {
											$hook_details['class'] = $hook_details['function'][0];
											$hook_details['function'] = $hook_details['function'][1];
										}
									}
